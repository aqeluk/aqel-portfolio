...
export default function Navbar() {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 z-10 w-full bg-orange-700 shadow-lg backdrop-blur duration-200 sm:translate-y-0 sm:transition-none ${
        ...
        >
          {PAGES.map(({ id, href }, i) => (
            <li key={i} className="relative flex gap-10 first:grow">
              ...
              {i === PAGES.length - 1 && (
                <a
                  className="absolute -right-16 bottom-2 sm:relative sm:-bottom-2 sm:right-0"
                  href="/resume"
                  target="_blank"
                  aria-label="Resume"
                >
